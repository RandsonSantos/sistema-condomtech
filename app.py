...
# 🛡️ Segurança adicional para cookies de sessão
app.config['SESSION_TYPE'] = 'filesystem'
app.config['SESSION_PERMANENT'] = False
app.config['SESSION_COOKIE_NAME'] = 'session'  # ✅ substitui uso obsoleto de app.session_cookie_name
app.config['SESSION_COOKIE_HTTPONLY'] = True
app.config['SESSION_COOKIE_SECURE'] = True  # Use HTTPS em produção
Session(app)
...
def editar_os(os_id):
    ...
    if request.method == 'POST':
        ...
        os.status = request.form.get('status', '').strip()
...
def nova_os_para_cliente(cliente_id):
    ...
    if request.method == 'POST':
        ...
        nova_os = OrdemServico(cliente_id=cliente.id, observacoes=observacoes, status='Aberta')
...
def empresa():
    ...
    if request.method == 'POST':
        ...
        empresa.nome = request.form.get('nome', '').strip()
...
def dashboard_principal():
    ...
    valor_total = sum(os.total or 0 for os in ordens)
    valor_aberto = sum(os.total or 0 for os in ordens if os.status and os.status.lower() == 'aberta')
    valor_finalizado = sum(os.total or 0 for os in ordens if os.status and os.status.lower() == 'finalizada')
    valor_pago = sum(os.total or 0 for os in ordens if os.status and os.status.lower() == 'pago')
    valor_cancelado = sum(os.total or 0 for os in ordens if os.status and os.status.lower() == 'cancelada')
...
def buscar_ordens():
    ...
    if status:
        filtros.append(OrdemServico.status.ilike(status))
    ...
    ordens_filtradas = query.all()
    valor_aberto = sum(os.total or 0 for os in ordens_filtradas if os.status and os.status.lower() == 'aberta')
    valor_pago = sum(os.total or 0 for os in ordens_filtradas if os.status and os.status.lower() == 'pago')
    valor_cancelado = sum(os.total or 0 for os in ordens_filtradas if os.status and os.status.lower() == 'cancelada')
    valor_finalizado = sum(os.total or 0 for os in ordens_filtradas if os.status and os.status.lower() == 'finalizada')
...
def listar_ordens_por_status(status):
    ...
    valor_total = sum(ordem.total or 0 for ordem in ordens)
...
def relatorio_mensal():
    ...
    def soma_por_status(status):
        return sum(os.total or 0 for os in ordens if os.status and os.status.lower() == status.lower())
